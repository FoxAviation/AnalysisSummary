 ...
 \item Allg. Lsg.: $y(t)=y_h(t)+y_p(t)$
  ...
  \begin{itemize}
  	...
  	\item $\overline{z} = x-iy$
		...
		\begin{itemize}
			\item Harmonische Reihe: $\sum_{k=1}^{\infty}\frac{1}{k}=+\infty; \sum_{k=1}^{\infty}\frac{1}{k^a}=0 \ \forall a \geq 2;$
			\item Potenzreihe: $\sum_{k=1}^{\infty}a_0q^k=\frac{a_0}{1-q} \ \forall |q| < 1$
			\item Logarithmus Reihe: $\sum_{k=1}^{\infty}(-1)^{k+1}\frac{x^k}{k}$
		\end{itemize}
