...
$f$ in $x$ (total) diff'bar $\Leftrightarrow \exists$ lineare Abbildung $L: \real^n \rightarrow \real^n, h \in \real^n \backslash \{0\}: \lim\limits_{h \rightarrow 0} \frac{f(x+h)-f(x)-Lh}{\lVert h \rVert} = \lim\limits_{h \rightarrow 0} \frac{f(x+h)-f(x)-\langle w,h \rangle}{\lVert h \rVert} = 0$
 ...
 \end{pmatrix}$
   ...
   \paragraph{Extrema/Kritische Punkte}
	$c \in M$ lokales Extremum $\Rightarrow c$ kritischer Punkt von $f: \nabla f(c)=0$\\
	$f : M \rightarrow \real$ zweimal statig diff'bar
   \begin{itemize}
	\item $f$ in $c$ lokales Minimum $\Rightarrow \nabla^2f(c)$ positiv semidefinit
	\item $f$ in $c$ lokales Maximum $\Rightarrow \nabla^2f(c)$ negativ semidefinit
	\item $\nabla f(c)=0 \wedge \nabla^2f(c)$ positiv definit $\Rightarrow$ f in c isol. lokal. Maximum
	\item $\nabla f(c)=0 \wedge \nabla^2f(c)$ negativ definit $\Rightarrow$ f in c isol. lokal. Minimum
	\item $\nabla f(c)=0 \wedge \nabla^2f(c)$ positive und negative EW $\Rightarrow f$ in $c$ Sattelpunkt
   \end{itemize}

	\paragraph{Mehrdim. Kettenregel}
	$f: \real^n \rightarrow \real^m, G: \real^m \rightarrow  \real^d: F$ in $c$und G an $y := F(c)$ stetig diff'bar $\Rightarrow$ $D(G \circ F)(c)=DG(y) \cdot DF(c)$
	
  \paragraph{Taylor in höheren Dimensionen}
%TODO: Implement

  \section{Mehrdimensionale Integralrechnung}
	\paragraph{Satz von Fubini}
	%TODO: Continue here
  % Sazu von Fubini?
