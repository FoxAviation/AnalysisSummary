...
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%+++++++++++++ by hydrogenoxyde and FoxAviation +++++++++++++
%++++ published on github.com/FoxAviation/AnalysisSummary +++
%++++++++ with special thanks to BigHilo and kechtel ++++++++
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
\pagestyle{fancy}
\fancyhf{}
\lhead{Analysis Merkhilfe WiSe 16/17}
\rhead{von hydrogenoxyde und FoxAviation auf GitHub}
\rfoot{Alle Angaben ohne Gewähr und Anspruch auf Vollständigkeit. Kompiliert am: \today\ \currenttime}
\lfoot{\thepage}
...
$\lim\limits_{n \rightarrow \infty} (a_n) \rightarrow 0$
  ...
  $\aseq$, $\bseq$ reelle Folgen, $\lim\limits_{n \rightarrow \infty} a_n = a$, $\lim\limits_{n \rightarrow \infty} b_n = b$
  \begin{itemize}
...
\end{itemize}
  ...
  \resizebox{.9\hsize}{!}{
    \begin{tabular}{c|c|c|c|c|c}
      ...
      $\sinh$ 	& $0$ & $\star$ & $\star$ & $\star$ & $\star$ \\
      $\cosh$ 	& $1$ & $\star$ & $\star$ & $\star$ & $\star$ \\
    \end{tabular}
}
...
\quad f'(c) := \lim\limits_{x \rightarrow c} \frac{f(x)-f(c)}{x-c}$
  ...
  %This paragraph makes no sense at this point
...
%TODO: Definitionsbereich einfügen
  ...
  \paragraph{Riemann-Integral}\hspace{0pt}\\
...
\textit{Für \textbf{AWP}:} $y(t_0)=y_0, y'(t_1)=y_1$ setze $t=t_1, t=t_2$ in allg. Lösung ein und bestimme $c_1,c_2$
 ...
 \item Allg. Lsg.: $y(t)=y_h(t)+y_p(t)$
  ...
  \begin{itemize}
  	...
  	\item $\lvert z \rvert = \sqrt{x^2+y^2} = \sqrt{z \cdot \overline{z}}$
  	\item  $\varphi=\arctan(\frac{x}{y})$, $r=\lvert z \rvert$, $x = r \cdot \cos(\varphi)$, $y = r \cdot \sin(\varphi)$
  	\item $\overline{z} = x-iy$
  	\item $z = r(\cos(\varphi) + i \sin(\varphi))=r \cdot e^{i\varphi}$
  	
        \end{itemize}

	\section{Koordinatentransformation}
		\paragraph{2D - Kreiskoordinaten}\hspace{0pt}\\
		$
			\begin{pmatrix} x\\y \end{pmatrix} \mapsto r \begin{pmatrix} \cos (\varphi) \\  \sin (\varphi) \end{pmatrix},
			\quad dxdy \mapsto r \cdot d\varphi dr
		$
		
		\paragraph{3D - Kugelkoordinaten}\hspace{0pt}\\
		$
		\begin{pmatrix} x\\y\\z \end{pmatrix} \mapsto r \begin{pmatrix} \sin (\theta) \cos (\varphi) \\  \sin (\theta) \sin (\varphi) \\ \cos(\theta) \end{pmatrix},
		\quad dxdydz \mapsto r^2 \sin(\theta) drd\theta d\varphi
		$
		
		\paragraph{3D - Zylinderkoordinaten}\hspace{0pt}\\
		$
		\begin{pmatrix} x\\y\\z \end{pmatrix} \mapsto \begin{pmatrix} r \cdot \cos (\varphi) \\  r \cdot \sin (\varphi) \\ z \end{pmatrix},
		\quad dxdydz \mapsto r \cdot drd\varphi dz
		$
...
\section{Sonstiges}
        ...
        \begin{tabular}{|c|c|c|}
          ...
          \hline
           $\frac{1}{\cos^2(x)}$                                & $\tan(x)$         	& $-\ln(\cos(x))$                           	\\ %\hline
           $\cosh(x) $ 											& $\sinh(x)$ 			& $\cosh(x)$									\\
           $\frac{1}{\cosh^2(x)}$                               & $\tanh(x)$        	& $\ln(\cosh(x))$                           	\\ %\hline
           ...
           $\frac{1}{\sqrt{x^2+1}}$                             & $arcsinh(x)$      	& $x \sinh^{-1}(x)-\sqrt{x^2+1}$            	\\ %\hline
           $\frac{1}{\sqrt{x-1}\sqrt{x+1}}$                     & $arccosh(x)$      	& $x \cosh^{-1}(x)-\sqrt{x-1}\sqrt{x+1}$    	\\ %\hline
		...
		\begin{itemize}
			\item $f \in \laO(g) \leftrightarrow \lim\limits_{x \rightarrow a} \frac{\lvert f(x) \rvert}{g(x)} < \infty$
			\item $f \in o(g) \leftrightarrow \lim\limits_{x \rightarrow a} \frac{\lvert f(x) \rvert}{g(x)} = 0$
			\item $f \in \Omega(g) \leftrightarrow \lim\limits_{x \rightarrow a} \frac{g(x)}{\lvert f(x) \rvert} < \infty$
			\item $f \in \omega(g) \leftrightarrow \lim\limits_{x \rightarrow a} \frac{g(x)}{\lvert f(x) \rvert} = 0$
		\end{itemize}
	...
	%TODO: Expand
%	\paragraph{Besondere Potenzreihe: $\sum_{k=1}^{\infty} a^kz^{k^2}$}
%	\begin{enumerate}
%		\item Umschreiben in $\sum_{k=1}^{\infty} c_kz^k$ mit $c_k=
%		\begin{cases}
%			a^m & \text{falls } k=a^m, \text{für ein } n \in \nat\\
%			0 & , \text{sonst}
%		\end{cases}$
%		\item $HP_1 = 0$\\Falls $a \neq 0: \sqrt[k^2]{\lvert a^k \rvert} = 1 \Rightarrow HP_2 = 1$, nach oben beschränkt\\
%		$R^{-1} = \begin{cases}
%		n & a \neq 0\\
%		\infty & a = 0
%		\end{cases}$
%		da $\limsup_{k \rightarrow \infty} \sqrt[k]{\lvert k_n \rvert} = \begin{cases}
%		1 & ,a \neq 0\\
%		0 & ,a = 0
%		\end{cases})$
%	\end{enumerate}
